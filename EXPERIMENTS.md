...
| 16 | [16-position-transport.md](experiments/16-position-transport.md) | heldout.py | can learned reads be made position-transportable by held-out-position checkpoint selection (arm A) or mixed-position training (arm B)? judged on unseen positions {10,14,22}; small write widening; ρ/EPR evaluation-only (honesty: clean reference is T-aware) | adjudication outcome (3): **clean-plane access is special; the entanglement is intrinsic** — no transportable checkpoint exists on any trajectory (w2's val gain never positive); mixed-position training *memorizes* the wider set (+27.7% at trained positions, +3.0% at interpolated); 3 of 4 near-plane writes diverge (population evidence, nearest-plane worst); P4 4th hold, agreement extends descriptively to unseen positions. Read-construction thread concluded: four repair families falsified; failure mode oracle-free detectable | concluded |
