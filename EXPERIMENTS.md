...
| 8 | [8-adversarial-coordinates.md](experiments/8-adversarial-coordinates.md) | adversarial.py | does interventional discovery survive adversarially ill-conditioned coordinates where PCA must fail? (closes exp 6/7's variance-mimicry limitation; constructed validation, exp-2 buried-cache tradition) | P1/P4/P5 hold, P2/P3 fail: VARIANCE DEPENDENCE EXPOSED — the loop's second-moment *proposal* miner chases amplified junk and dies at k\*=0, while behavioral *acceptance* stays sound (rejected every junk direction; reported its own failure honestly). Exp 6/7's discoveries were proposal-variance-lucky; the scale-free miner repair is the next registration. Surprises: pls-z flips echo→85% causal; rand-z is destructive (−427%, pullback off-manifold amplification) | concluded |
