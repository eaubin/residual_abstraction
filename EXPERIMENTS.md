...
| 26 | [26-reference-consolidation.md](experiments/26-reference-consolidation.md) | scripts/oracle_withdrawal/reference_consolidation.py | Oracle-withdrawal 4: pre-Block-3 consolidation on `pstack` (8 fresh seeds) — is `cegar`≈`pca` (mimicry) with proper multi-seed statistics, and do the Dyck-transferred ρ bands (0.25/0.5) calibrate on `pstack` under the single `cegar` anchor, with the reference estimates exact-coherent? | — | pre-registered |
