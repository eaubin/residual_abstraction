...
| 24 | [24-hidden-reference.md](experiments/24-hidden-reference.md) | scripts/oracle_withdrawal/reference_selection.py | Oracle-withdrawal 2: can observable diagnostics select a compact trusted reference on `pstack` before exact audit is revealed? | **P2 fails informatively: `REFERENCE_AMBIGUITY_CONFIRMED`, NO-GO** — three distinct `k=4` references (`cegar`/`pca`/`delta`) tie at ~93% closure observably AND exactly (spread 0.003) yet span subspaces 10–12° apart; near-equal closure ≠ same reference (opposite pole from Mess3 variance-mimicry). Subspace criterion (F2) load-bearing: closure spread alone read BENIGN. No unique earned reference → register tie-break follow-up (exp 25) before battery transfer | result review |
