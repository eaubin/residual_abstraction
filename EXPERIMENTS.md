...
| 10 | [10-interventional-search.md](experiments/10-interventional-search.md) | candidates.py | finite-candidate interventional search: 12-candidate pools from cheap fallible sources (incl. the exp-9 losers), ranked by *measured closure gain under patching*, acceptance unchanged, uniform working-coordinate patch convention judged after pullback; adjudicates selection-works / support-or-patch-failure / mere-reranking via registered per-candidate gain+angle tables; gradients deferred to exp 11 | P1/P2/P6 hold; P3 fails; P4/P5 NOT TESTED. Trichotomy adjudicates to **2b: patch-parameterization failure** — the adversarial pool reached the plane (best candidate 1.1°, the exp-9 loser's other back-mapping) yet best gain was +1.5%, with near-plane randoms catastrophically destructive (−105/−187%): the working-coordinate orthogonal swap's *read side* is junk-amplified ×κ (needs contamination ≲ κ⁻² to behave). Benignly, selection works and is not reranking (dPCA1 beat M1 by measured gain; k\*=2 at 98.4%). The exp-3 interchange patch is the remaining coordinate-lucky component; patches must be co-selected with directions → exp 11, with gradients explicitly warned | concluded |
