...
| 24 | [24-hidden-reference.md](experiments/24-hidden-reference.md) | scripts/oracle_withdrawal/reference_selection.py | Oracle-withdrawal 2: can observable diagnostics select a compact trusted reference on `pstack` before exact audit is revealed? | **P2 fails informatively: `REFERENCE_AMBIGUITY_CONFIRMED`, NO-GO** — three `k=4` candidates (`cegar`/`pca`/`delta`) tie at ~93% closure observably AND exactly (spread 0.003), but resolve into two distinct references: `pca` separates (10.4°/12.4°) from a near-coincident `{cegar,delta}` plane (4.9°). Near-equal closure ≠ same reference (opposite pole from Mess3 variance-mimicry); CONFIRMED driven by `pca` alone, ~2.4° over the 10° line. Subspace criterion (F2) load-bearing: closure spread alone read BENIGN. Exact audit indifferent (0.926/0.923/0.923) ⇒ no "right" reference to recover; auditable follow-up (exp 25) is downstream-ρ invariance across the equally-good references, gated on seed-stability of the single-seed split. **[Exp 25 back-annotation: the `pca`-vs-`{cegar,delta}` split is seed-fragile — `pca` is a clean outlier in only 3/8 seeds; the ambiguity is a marginal-seed artifact, and `pstack` has one stable ~k=4 reference]** | concluded |
