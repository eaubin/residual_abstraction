...
| 16 | [16-position-transport.md](experiments/16-position-transport.md) | heldout.py | can learned reads be made position-transportable by held-out-position checkpoint selection (arm A) or mixed-position training (arm B)? judged on unseen positions {10,14,22}; small write widening; ρ/EPR evaluation-only (honesty: clean reference is T-aware) | pre-registered P1–P8 (P2 ~45%, P3 ~50%, P7 w1-rescue ~30%) | pre-registered, not run |
