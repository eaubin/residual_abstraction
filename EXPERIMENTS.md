...
| 24 | [24-hidden-reference.md](experiments/24-hidden-reference.md) | scripts/oracle_withdrawal/reference_selection.py | Oracle-withdrawal 2: can observable diagnostics select a compact trusted reference on `pstack` before exact audit is revealed? | **P2 fails informatively: `REFERENCE_AMBIGUITY_CONFIRMED`, NO-GO** — three `k=4` candidates (`cegar`/`pca`/`delta`) tie at ~93% closure observably AND exactly (spread 0.003), but resolve into two distinct references: `pca` separates (10.4°/12.4°) from a near-coincident `{cegar,delta}` plane (4.9°). Near-equal closure ≠ same reference (opposite pole from Mess3 variance-mimicry); CONFIRMED driven by `pca` alone, ~2.4° over the 10° line. Subspace criterion (F2) load-bearing: closure spread alone read BENIGN. Exact audit indifferent (0.926/0.923/0.923) ⇒ no "right" reference to recover; auditable follow-up (exp 25) is downstream-ρ invariance across the equally-good references, gated on seed-stability of the single-seed split. **[Exp 25 back-annotation: the tie REPRODUCES (G1 7/8, NO-GO stands), but the "two distinct references" structure does NOT — `pca` is a clean outlier in only 3/8 seeds; cross-seed the tied candidates are one cluster, so the honest label is BENIGN not CONFIRMED. `pstack` has one stable ~k=4 reference; the distinctness rode seed-0's marginal angle]** | concluded |
| 25 | [25-rho-invariance.md](experiments/25-rho-invariance.md) | scripts/oracle_withdrawal/rho_invariance.py | Oracle-withdrawal 3: does the exp-24 reference ambiguity matter downstream for ρ? Arm A seed-stability gate on the `{pca\|cegar,delta}` split (8 seeds), then Arm B ρ anchor-invariance (does the equivalence verdict depend on whether `pca` or `cegar` anchors ρ?) | **`SEED_UNSTABLE_CLUSTER`, NO-GO** — the two-reference split is sampling noise: `pca` is a clean 10° outlier in only 3/8 seeds (joint tie∧outlier 2/8 ≪ 6), `pca`–`cegar` straddling 10° (6.9–13.9°), while `cegar`–`delta` stays one cluster (2–7°). One stable ~k=4 reference; ρ-invariance moot (Arm B not run). Recontextualizes exp 24's ambiguity as a marginal-seed artifact; a disclosed 3-seed peek (seeds 0–2) was the unrepresentative favorable tail. Battery transfer under the single `cegar` core → exp 26 | concluded |
| 26 | [26-reference-consolidation.md](experiments/26-reference-consolidation.md) | scripts/oracle_withdrawal/reference_consolidation.py | Oracle-withdrawal 4: pre-Block-3 consolidation on `pstack` (8 fresh seeds) — is `cegar`≈`pca` (mimicry) with proper multi-seed statistics, and does ρ separate the extreme known cases under the single `cegar` anchor (validating ρ's separation, not the Dyck-inherited threshold values), with the reference estimates exact-coherent? Splits `RHO_OVERSENSITIVE` (NO-GO) from `BANDS_RECALIBRATE` (GO) | **GO to Block 3** — one coherent stable reference (closures 0.91–0.94, `cegar` std 0.005); ρ separates extremes cleanly (`BANDS_TRANSFER`, separation 0.830 ≈ Dyck), `RHO_OVERSENSITIVE` did not fire; `cegar`–`pca` `PARTIAL_MIMICRY` (mean 10.0°, vs Mess3 3.5° / random 87.8°) but the ~10° gap is behaviorally inert at ρ (estimates equivalent at 0.02–0.04) — geometric near-distinctness ≠ behavioral, so the honest object is one behavioral reference. ρ band *values* stay Dyck-inherited (no intermediate known-case). → exp 27 battery transfer | concluded |
