...
standard PairSet self-checks pass, the global observable scale is
non-degenerate (`D0 > D_full`), and both stratified guards pass
(`STRATA_TOO_SPARSE`, `STRATA_VACUOUS` above). Any failure = typed
substrate halt (status `1`), not an Exp 24 reference-selection result;
register a substrate repair.
...
Not run. Pause for pre-run review of the selection rule and code.
