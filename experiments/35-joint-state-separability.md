...
- **bundle co-movement** `c(other)` — signed control of the other bundle
  predicate; the joint reading wants this positive and large.
...
precedence order. Gates require the condition for **both** arms.
...
`OOB_ROOM_MIN=0.01`, `MGRAM_MAX=0.85`, `OE_BAND=0.10`, `SEED_MAJORITY=3`.
...
- **P2 (room/calibration; likely).** Both bundle predicates retain exp-34 room
  ...
  specificity floor). If `phi4` room is marginal, `NO_OOB_ROOM` is a clean,
  honest non-diagnostic outcome.
...
| `phi4` happens to be flat/low-room so "spared" is vacuous | `NO_OOB_ROOM` gate; `phi4` room reported |
...
- `COUPLE_MIN=0.40`: the other bundle predicate must close at least 40% of its
  gap toward source for the move to count as joint; set below `C_MIN` because
  co-movement is a secondary, not the matched-on, target.
...
- `OE_BAND=0.10`: inherited endpoint audit band; exp 34 observed `~0.01`.

## Measured-but-Unadjudicated

The script prints, per seed: the read-geometry cosines; per arm the selected
disc/held alpha, target control, bundle co-movement, out-of-bundle control,
m-gram closure, own-delta ceiling, mismatched/shuffled floors, room, `phi4`
room, audit, and retention. The verdict reads only the quantities in
`classify_seed`. The read-geometry cosines and dose curves are corroborating
context, not adjudicated. Selected matched alphas above `1.0` are reported as a
quality caveat — exp 34 saw `alpha>1` for `phi2`; a joint-variable result whose
selectivity only appears at extrapolation strength is a direction-level result,
noted in the conclusion, not silently promoted.
...
- Vacuous "spared": guarded by `NO_OOB_ROOM` and the reported `phi4` room.
