...
| fractional-precision reads c ∝ Σ̂^{−α}w (exp 12) | floored eigendecomposition (10⁻¹⁰·λ_max) of the *sample* covariance; and the registered impossibility note — no α equals the clean read unless the x-spectrum is flat on the relevant directions | **falsified as a sufficient family** (exp 12: adversarial gains flat ~+1.5% across the whole grid; the obstacle is neutral-background read contamination, not junk — junk suppression succeeds by α ≥ 0.75 and transfer never appears; prec confirmed behaviorally equivariant, +1.4% = +1.4% same-write). Clean-read *composition* exonerated (D2: 97.8%); the open object is a non-spectral read construction |
