...
| the clean / T-aware patch can serve as a *trusted reference* (for ρ) | the battery's ρ needs a reference; at LLM scale it must be the best-validated patch, not an oracle | **supported** on toys (exps 15–18, incl. the κ = 30 positive-direction case and horizon stability); LLM-scale transport **open** |
