...
| pstack `phi1`/`phi2` are facets of one coupled stack-state variable | decides whether exp-34/35 "non-specificity" is correct identification of one variable or broad replacement; a backward-design input for the next toy's predicate inventory | **open — valence uncertain** — causally coupled (a directed near-manifold delta co-moves both beyond no-information floors, 4/4 seeds); but pstack cannot tell *one variable* from *broad replacement* (no high-room, known-out-of-bundle control). Hand forward as a question to understand, not a settled claim. **exp 40 (Dyck-2, the well-posed analogue: `depth`/`top_type`) exhibits an *analogous* coupling on a toy whose facets are separable by construction, adding structure — *asymmetric* and *depth-graded*: the `top_type` difference-direction steers cleanly (no drag on depth), but the depth direction drags `top_type` (stable `CROSS_DRAG` at k=2, 4/4; borderline/`SEED_UNSTABLE` at k=1), larger at the deeper contrast. Consistent with the coupling being representational rather than a `pstack`-specific artifact — but on a *different process* (not a direct resolution of `pstack`) and only for the rank-1 additive direction class. The one-variable-vs-broad-replacement question stays open in a sharper form: `v_depth` is non-orthogonal to the type direction, which the data cannot separate from a causal sum→ratio coupling.** exps 34–36, 40 |
