...
| pstack `phi1`/`phi2` are facets of one coupled stack-state variable | decides whether exp-34/35 "non-specificity" is correct identification of one variable or broad replacement; a backward-design input for the next toy's predicate inventory | **open — valence uncertain** — causally coupled (a directed near-manifold delta co-moves both beyond no-information floors, 4/4 seeds); but pstack cannot tell *one variable* from *broad replacement* (no high-room, known-out-of-bundle control). Hand forward as a question to understand, not a settled claim. **exp 40 (Dyck-2, the well-posed analogue: `depth`/`top_type`) reproduces the coupling on a clean separating toy and adds structure — it is *asymmetric* and *depth-graded*: the `top_type` difference-direction steers cleanly (no drag on depth), but the depth direction drags `top_type` (stable `CROSS_DRAG` at k=2, 4/4), drag growing with stack depth. So the coupling is representational, not a toy artifact — for this rank-1 additive direction class. The one-variable-vs-broad-replacement (here: causal sum→ratio coupling vs non-orthogonal difference-directions) question stays open: `v_depth` is non-orthogonal to the type direction, which the data cannot separate from causation.** exps 34–36, 40 |
