...
| pstack `phi1`/`phi2` are facets of one coupled stack-state variable | decides whether exp-34/35 "non-specificity" is correct identification of one variable or broad replacement; a backward-design input for the next toy's predicate inventory | **open — valence uncertain** — causally coupled on pstack (directed near-manifold delta co-moves both, 4/4; cannot separate *one variable* from *broad replacement*, no high-room out-of-bundle control). exp 40 reproduces an *analogous*, now **asymmetric and depth-graded** coupling on a separable-by-construction Dyck-2 toy. **exp 42 characterized that Dyck-2 drag as curvature-dominated / linear-limit-separable (geometric); a representational depth-conditional readout was *not* established** (the gradient rotation is not distinguished from generic readout nonlinearity) — so the Dyck-2 analog leans **geometric**, not representational. **exp 43 + the state-localization close (exp 44) add the architecture-level reading:** on Dyck-2 the two facets are read through a shared bracket-readout head (`(3,attn,3)@t+k`) — same parts, entangled — consistent with the geometric (not representational) coupling. The `pstack` causal-vs-geometric question itself stays **open** (different process; not a `pstack` resolution); ledger schema left as-is at the phase close (no process-agnostic promotion this cycle). exps 34–36, 40, 42, 43, 44 |
