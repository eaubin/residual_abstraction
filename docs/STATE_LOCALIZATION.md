...
- **L-int — directional specificity / interventional dissociation (exp 40, the
  ...
  *directions* separable? Steer each facet by a rank-1-per-position additive
  ...
  floor decides `DISSOCIATED` (diagonal-dominant — directions act independently) vs
  `CROSS_DRAG` (off-diagonal — representational coupling) vs `MIXED`/`NO_HANDLE`.
  Full replacement is tautological for specificity (37 transport guard), so the
  intervention is sub-residual/additive.
...
- **L2 — coarse localization (deferred behind L-int).** Block granularity, layer
  ...
  locus. Re-motivated only by a `DISSOCIATED`/`MIXED` outcome at L-int — it localizes
